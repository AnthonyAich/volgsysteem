        ...
        <>
            ...
            <Navbar />
            {
                groepNaam && (<main className="px-6 lg:mx-28 lg:px-40 bg-white w-auto h-fit lg:h-screen pt-24">
                    ...
                    {
                        assignmentToDelete && <DeleteOpdrachtPopup assignmentName={assignmentsState[0].naam} opdrachtId={assignmentToDelete} setAssignmentsState={setAssignmentsState} />
                    }
                    {
...
const DeleteOpdrachtPopup = ({ assignmentName, opdrachtId, setAssignmentsState }) => {
    const deleteHandler = (id) => {
    ...
    return (
        <>
            <div className="cursor-pointer fixed w-screen top-0 left-0 z-50 p-4 md:inset-0 md:h-full bg-opacity-70 bg-darkGrey flex justify-center items-center" onClick={() => setAssignmentsState([])}>
            </div>
            <div className="w-fit fixed left-1/2 bottom-1/2 translate-y-1/2 -translate-x-1/2 z-50 h-fit max-w-md md:h-auto ">
                <div className="bg-white rounded-lg">
                    <button onClick={() => setAssignmentsState([])} type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
                        <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                        <span className="sr-only">Modal sluiten</span>
                    </button>
                    <div className="p-6 text-center">
                        ...
                        <button onClick={() => setAssignmentsState([])} type="button" className="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-lightGrey text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Nee, ik wil het niet
                        </button>
                    </div>
                </div>
            </div>
